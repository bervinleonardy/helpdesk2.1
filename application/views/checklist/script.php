...
<script>
  function tampildatachecklist() {
    $('#centangSemua').prop('checked', false);

    table = $('#dataChecklist').DataTable({
      responsive: true,
      "destroy": true,
      "processing": true,
      "serverSide": true,
      ...
      "ajax": {
        "url": "<?= site_url('ict/getDataChecklist') ?>",
        "type": "POST"
      },
  ...
  function edit(id) {
    $.ajax({
      type: 'post',
      url: "<?= site_url('ict/formeditchecklist'); ?>",
      data: {
        id: id
      },
      dataType: "json",
  ...
  function hapus(id, name) {
    Swal.fire({
      title: 'Delete',
      text: `Are you sure delete item checklist : ${name} ?`,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: 'post',
          url: "<?= site_url('ict/hapuschecklist'); ?>",
          data: {
            id: id
          },
          dataType: "json",
  ...
  $(document).ready(function() {
    ...
    $('.formhapus').submit(function(e) {
      ...
      if (jmldata.length === 0) {
        ...
        Swal.fire({
          title: 'Delete Data',
          text: `There's ${jmldata.length} items checklist will be delete ?`,
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
    ...
    $('#tomboltambah').click(function(e) {
      $.ajax({
        url: "<?= site_url('ict/formtambahchecklist'); ?>",
        dataType: "json",
        success: function(response) {
          if (response.sukses) {
            $('.viewmodal').html(response.sukses).show();
