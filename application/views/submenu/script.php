...
<script>
  function tampildata() {
    ...
    table = $('#data').DataTable({
      ...
      "ajax": {
        "url": "<?= site_url('menu/getDataSubmenu') ?>",
        "type": "POST"
      },
  ...
  function hapus(id, name) {
    Swal.fire({
      title: 'Delete',
      text: `Are you sure delete submenu : ${name} ?`,
      icon: 'warning',
  ...
  $(document).ready(function() {
    ...
    $('.formhapus').submit(function(e) {
      ...
      if (jmldata.length === 0) {
        ...
        Swal.fire({
          title: 'Delete Data',
          text: `There's ${jmldata.length} data submenu will be delete ?`,
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
    ...
    $('#tomboltambah').click(function(e) {
      $.ajax({
        url: "<?= site_url('menu/formtambahsubmenu'); ?>",
        dataType: "json",
        success: function(response) {
          if (response.sukses) {
            $('.viewmodal').html(response.sukses).show();
