...
<script>
  function tampildatacases() {

    $('#centangSemua').prop('checked', false);

    table = $('#dataCases').DataTable({
      responsive: true,
      "destroy": true,
      "processing": true,
      "serverSide": true,
      ...
      "ajax": {
        "url": "<?= site_url('administrator/getDataCases') ?>",
        "type": "POST"
      },
  ...
  function edit(id) {
    $.ajax({
      type: 'post',
      url: "<?= site_url('administrator/formeditcases'); ?>",
      data: {
        id: id
      },
      dataType: "json",
  ...
  function hapus(id, name) {
    Swal.fire({
      title: 'Delete',
      text: `Are you sure delete case : ${name} ?`,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: 'post',
          url: "<?= site_url('administrator/hapuscases'); ?>",
          data: {
            id: id
          },
          dataType: "json",
  ...
  $(document).ready(function() {
    ...
    $('.formhapus').submit(function(e) {
      ...
      if (jmldata.length === 0) {
        ...
        Swal.fire({
          title: 'Delete Data',
          text: `There's ${jmldata.length} data Cases will be delete ?`,
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
    ...
    $('#tomboltambah').click(function(e) {
      $.ajax({
        url: "<?= site_url('administrator/formtambahcases'); ?>",
        dataType: "json",
        success: function(response) {
          if (response.sukses) {
            $('.viewmodal').html(response.sukses).show();
