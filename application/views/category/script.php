...
<script>
  function tampildatacategory() {
    ...
    table = $('#dataCategory').DataTable({
      ...
      "ajax": {
        "url": "<?= site_url('administrator/getDataCategory') ?>",
        "type": "POST"
      },
  ...
  function edit(id) {
    $.ajax({
      type: 'post',
      url: "<?= site_url('administrator/formeditcategory'); ?>",
      data: {
        id: id
      },
      dataType: "json",
  ...
  function hapus(id, name) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: 'post',
          url: "<?= site_url('administrator/hapuscategory'); ?>",
          data: {
            id: id
          },
          dataType: "json",
  ...
  $(document).ready(function() {
    ...
    $('.formhapus').submit(function(e) {
      ...
      if (jmldata.length === 0) {
        ...
        Swal.fire({
          title: 'Delete Data',
          text: `There's ${jmldata.length} data category will be delete ?`,
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
    ...
    $('#tomboltambah').click(function(e) {
      $.ajax({
        url: "<?= site_url('administrator/formtambahcategory'); ?>",
        dataType: "json",
        success: function(response) {
          if (response.sukses) {
            $('.viewmodal').html(response.sukses).show();
